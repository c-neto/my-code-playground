...
from sprintreports.jira import IssuesCollectionDTO
...
def get_area(issue: dict, issue_custom_field_area: str):
    issue_area = py_.get(issue, f"fields.{issue_custom_field_area}[0].value")
...
def get_epic_link_summary(issue: dict):
    ...
    issue_type = py_.get(issue, "issue.fields.parent.fields.issuetype.name")

    if py_.lower_case(issue_type) == "epic":
...
def get_team(issue: dict, issue_custom_field_team: str):
    issue_team = py_.get(issue, f"fields.{issue_custom_field_team}[0].value")
...
def _make_issue_row(issue: dict, is_planned: bool, sprint: int):
    ...
    area = get_area(issue, settings["issue_custom_field"]["area"])
    board = get_board(issue, settings["board_by_status_mapping"])
    epic_link_summary = get_epic_link_summary(issue)
    team = get_team(issue, settings["issue_custom_field"]["team"])
